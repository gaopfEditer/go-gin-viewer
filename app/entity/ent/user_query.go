...
type UserQuery struct {
	...
	withProducts       *ProductManagerQuery
...
func (uq *UserQuery) QueryProducts() *ProductManagerQuery {
	query := (&ProductManagerClient{config: uq.config}).Query()
...
// QueryCreatedDevices chains the current query on the "created_devices" edge.
func (uq *UserQuery) QueryCreatedDevices() *DeviceQuery {
...
// QueryUpdatedDevices chains the current query on the "updated_devices" edge.
func (uq *UserQuery) QueryUpdatedDevices() *DeviceQuery {
	query := (&DeviceClient{config: uq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uq.prepareQuery(ctx); err != nil {
			return nil, err
...
func (uq *UserQuery) WithProducts(opts ...func(*ProductManagerQuery)) *UserQuery {
	query := (&ProductManagerClient{config: uq.config}).Query()
...
// WithCreatedDevices tells the query-builder to eager-load the nodes that are connected to
// the "created_devices" edge. The optional arguments are used to configure the query builder of the edge.
func (uq *UserQuery) WithCreatedDevices(opts ...func(*DeviceQuery)) *UserQuery {
	query := (&DeviceClient{config: uq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	uq.withCreatedDevices = query
	return uq
}

// WithUpdatedDevices tells the query-builder to eager-load the nodes that are connected to
// the "updated_devices" edge. The optional arguments are used to configure the query builder of the edge.
func (uq *UserQuery) WithUpdatedDevices(opts ...func(*DeviceQuery)) *UserQuery {
	query := (&DeviceClient{config: uq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (uq *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := uq.withProducts; query != nil {
		if err := uq.loadProducts(ctx, query, nodes,
			func(n *User) { n.Edges.Products = []*ProductManager{} },
			func(n *User, e *ProductManager) { n.Edges.Products = append(n.Edges.Products, e) }); err != nil {
			return nil, err
		}
	}
	...
	if query := uq.withCreatedDevices; query != nil {
		if err := uq.loadCreatedDevices(ctx, query, nodes,
			func(n *User) { n.Edges.CreatedDevices = []*Device{} },
			func(n *User, e *Device) { n.Edges.CreatedDevices = append(n.Edges.CreatedDevices, e) }); err != nil {
			return nil, err
		}
	}
	if query := uq.withUpdatedDevices; query != nil {
...
func (uq *UserQuery) loadProducts(ctx context.Context, query *ProductManagerQuery, nodes []*User, init func(*User), assign func(*User, *ProductManager)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(productmanager.FieldUserID)
	}
...
func (uq *UserQuery) loadAuditLogs(ctx context.Context, query *AuditLogQuery, nodes []*User, init func(*User), assign func(*User, *AuditLog)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(auditlog.FieldOperatorID)
	}
...
func (uq *UserQuery) loadCreatedDevices(ctx context.Context, query *DeviceQuery, nodes []*User, init func(*User), assign func(*User, *Device)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.Where(predicate.Device(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.CreatedDevicesColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
func (uq *UserQuery) loadUpdatedDevices(ctx context.Context, query *DeviceQuery, nodes []*User, init func(*User), assign func(*User, *Device)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.Where(predicate.Device(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.UpdatedDevicesColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
