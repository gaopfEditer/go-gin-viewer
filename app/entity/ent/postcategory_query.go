...
func (pcq *PostCategoryQuery) FirstID(ctx context.Context) (id int, err error) {
	var ids []int
...
func (pcq *PostCategoryQuery) FirstIDX(ctx context.Context) int {
...
func (pcq *PostCategoryQuery) OnlyID(ctx context.Context) (id int, err error) {
	var ids []int
...
func (pcq *PostCategoryQuery) OnlyIDX(ctx context.Context) int {
...
func (pcq *PostCategoryQuery) IDs(ctx context.Context) (ids []int, err error) {
...
func (pcq *PostCategoryQuery) IDsX(ctx context.Context) []int {
	ids, err := pcq.IDs(ctx)
	if err != nil {
		panic(err)
	}
...
func (pcq *PostCategoryQuery) loadPosts(ctx context.Context, query *PostQuery, nodes []*PostCategory, init func(*PostCategory), assign func(*PostCategory, *Post)) error {
	...
	nodeids := make(map[int]*PostCategory)
	for i := range nodes {
	...
	for _, n := range neighbors {
		fk := n.post_category_posts
		if fk == nil {
			return fmt.Errorf(`foreign-key "post_category_posts" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "post_category_posts" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
...
func (pcq *PostCategoryQuery) querySpec() *sqlgraph.QuerySpec {
	_spec := sqlgraph.NewQuerySpec(postcategory.Table, postcategory.Columns, sqlgraph.NewFieldSpec(postcategory.FieldID, field.TypeInt))
	_spec.From = pcq.sql
