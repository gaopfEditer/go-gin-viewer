...
type PostQuery struct {
	...
	withCategory     *PostCategoryQuery
	withTagRelations *PostTagRelationQuery
...
func (pq *PostQuery) QueryCategory() *PostCategoryQuery {
...
// QueryTagRelations chains the current query on the "tag_relations" edge.
func (pq *PostQuery) QueryTagRelations() *PostTagRelationQuery {
	query := (&PostTagRelationClient{config: pq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := pq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (pq *PostQuery) Clone() *PostQuery {
	...
	return &PostQuery{
		...
		withTagRelations: pq.withTagRelations.Clone(),
...
func (pq *PostQuery) WithCategory(opts ...func(*PostCategoryQuery)) *PostQuery {
	query := (&PostCategoryClient{config: pq.config}).Query()
...
// WithTagRelations tells the query-builder to eager-load the nodes that are connected to
// the "tag_relations" edge. The optional arguments are used to configure the query builder of the edge.
func (pq *PostQuery) WithTagRelations(opts ...func(*PostTagRelationQuery)) *PostQuery {
	query := (&PostTagRelationClient{config: pq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	pq.withTagRelations = query
	return pq
}
...
func (pq *PostQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Post, error) {
	var (
		...
		loadedTypes = [3]bool{
			pq.withCategory != nil,
			pq.withTagRelations != nil,
			pq.withAuthor != nil,
		}
	)
	if pq.withCategory != nil || pq.withAuthor != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := pq.withCategory; query != nil {
		if err := pq.loadCategory(ctx, query, nodes, nil,
			func(n *Post, e *PostCategory) { n.Edges.Category = e }); err != nil {
			return nil, err
		}
	}
	if query := pq.withTagRelations; query != nil {
		if err := pq.loadTagRelations(ctx, query, nodes,
			func(n *Post) { n.Edges.TagRelations = []*PostTagRelation{} },
			func(n *Post, e *PostTagRelation) { n.Edges.TagRelations = append(n.Edges.TagRelations, e) }); err != nil {
			return nil, err
		}
	}
...
func (pq *PostQuery) loadCategory(ctx context.Context, query *PostCategoryQuery, nodes []*Post, init func(*Post), assign func(*Post, *PostCategory)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Post)
	for i := range nodes {
		if nodes[i].post_category_posts == nil {
			continue
		}
		fk := *nodes[i].post_category_posts
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "post_category_posts" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (pq *PostQuery) loadTagRelations(ctx context.Context, query *PostTagRelationQuery, nodes []*Post, init func(*Post), assign func(*Post, *PostTagRelation)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(posttagrelation.FieldPostID)
	}
	query.Where(predicate.PostTagRelation(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(post.TagRelationsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
