...
type PostTagQuery struct {
	...
	withPostRelations *PostTagRelationQuery
...
// QueryPostRelations chains the current query on the "post_relations" edge.
func (ptq *PostTagQuery) QueryPostRelations() *PostTagRelationQuery {
	query := (&PostTagRelationClient{config: ptq.config}).Query()
...
func (ptq *PostTagQuery) FirstID(ctx context.Context) (id int, err error) {
	var ids []int
...
func (ptq *PostTagQuery) FirstIDX(ctx context.Context) int {
...
func (ptq *PostTagQuery) OnlyID(ctx context.Context) (id int, err error) {
	var ids []int
...
func (ptq *PostTagQuery) OnlyIDX(ctx context.Context) int {
...
func (ptq *PostTagQuery) IDs(ctx context.Context) (ids []int, err error) {
...
func (ptq *PostTagQuery) IDsX(ctx context.Context) []int {
	ids, err := ptq.IDs(ctx)
	if err != nil {
		panic(err)
	}
...
func (ptq *PostTagQuery) Clone() *PostTagQuery {
	...
	return &PostTagQuery{
		...
		withPostRelations: ptq.withPostRelations.Clone(),
...
// WithPostRelations tells the query-builder to eager-load the nodes that are connected to
// the "post_relations" edge. The optional arguments are used to configure the query builder of the edge.
func (ptq *PostTagQuery) WithPostRelations(opts ...func(*PostTagRelationQuery)) *PostTagQuery {
	query := (&PostTagRelationClient{config: ptq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	ptq.withPostRelations = query
	return ptq
}
...
func (ptq *PostTagQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*PostTag, error) {
	...
	if query := ptq.withPostRelations; query != nil {
		if err := ptq.loadPostRelations(ctx, query, nodes,
			func(n *PostTag) { n.Edges.PostRelations = []*PostTagRelation{} },
			func(n *PostTag, e *PostTagRelation) { n.Edges.PostRelations = append(n.Edges.PostRelations, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (ptq *PostTagQuery) loadPostRelations(ctx context.Context, query *PostTagRelationQuery, nodes []*PostTag, init func(*PostTag), assign func(*PostTag, *PostTagRelation)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*PostTag)
	for i := range nodes {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(posttagrelation.FieldPostTagID)
	}
	query.Where(predicate.PostTagRelation(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(posttag.PostRelationsColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		...
		node, ok := nodeids[fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "post_tag_id" returned %v for node %v`, fk, n.ID)
		}
		assign(node, n)
	}
	return nil
...
func (ptq *PostTagQuery) querySpec() *sqlgraph.QuerySpec {
	_spec := sqlgraph.NewQuerySpec(posttag.Table, posttag.Columns, sqlgraph.NewFieldSpec(posttag.FieldID, field.TypeInt))
	_spec.From = ptq.sql
