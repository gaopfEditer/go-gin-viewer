...
func (alq *AuditLogQuery) QueryOperator() *UserQuery {
...
func (alq *AuditLogQuery) QueryProduct() *ProductQuery {
	query := (&ProductClient{config: alq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := alq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (alq *AuditLogQuery) IDs(ctx context.Context) (ids []int, err error) {
...
func (alq *AuditLogQuery) Clone() *AuditLogQuery {
	...
	return &AuditLogQuery{
		...
		withOperator: alq.withOperator.Clone(),
		withProduct:  alq.withProduct.Clone(),
...
func (alq *AuditLogQuery) WithOperator(opts ...func(*UserQuery)) *AuditLogQuery {
	query := (&UserClient{config: alq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	alq.withOperator = query
	return alq
}

// WithProduct tells the query-builder to eager-load the nodes that are connected to
// the "product" edge. The optional arguments are used to configure the query builder of the edge.
func (alq *AuditLogQuery) WithProduct(opts ...func(*ProductQuery)) *AuditLogQuery {
	query := (&ProductClient{config: alq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	alq.withProduct = query
	return alq
}
...
//		GroupBy(auditlog.FieldOperatorID).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
//
//	client.AuditLog.Query().
//		Select(auditlog.FieldOperatorID).
//		Scan(ctx, &v)
...
func (alq *AuditLogQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*AuditLog, error) {
	...
	if query := alq.withOperator; query != nil {
		if err := alq.loadOperator(ctx, query, nodes, nil,
			func(n *AuditLog, e *User) { n.Edges.Operator = e }); err != nil {
			return nil, err
		}
	}
	if query := alq.withProduct; query != nil {
		if err := alq.loadProduct(ctx, query, nodes, nil,
			func(n *AuditLog, e *Product) { n.Edges.Product = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (alq *AuditLogQuery) loadOperator(ctx context.Context, query *UserQuery, nodes []*AuditLog, init func(*AuditLog), assign func(*AuditLog, *User)) error {
...
func (alq *AuditLogQuery) loadProduct(ctx context.Context, query *ProductQuery, nodes []*AuditLog, init func(*AuditLog), assign func(*AuditLog, *Product)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*AuditLog)
	for i := range nodes {
...
func (alq *AuditLogQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := alq.ctx.Fields; len(fields) > 0 {
		...
		if alq.withOperator != nil {
			_spec.Node.AddColumnOnce(auditlog.FieldOperatorID)
		}
		if alq.withProduct != nil {
			_spec.Node.AddColumnOnce(auditlog.FieldProductID)
		}
	}
