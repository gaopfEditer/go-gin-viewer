...
func (dq *DeviceQuery) QueryLicenseType() *LicenseTypeQuery {
	query := (&LicenseTypeClient{config: dq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := dq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (dq *DeviceQuery) QueryCreator() *UserQuery {
...
func (dq *DeviceQuery) QueryUpdater() *UserQuery {
	query := (&UserClient{config: dq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := dq.prepareQuery(ctx); err != nil {
			return nil, err
...
func (dq *DeviceQuery) Clone() *DeviceQuery {
	...
	return &DeviceQuery{
		...
		withLicenseType: dq.withLicenseType.Clone(),
...
func (dq *DeviceQuery) WithLicenseType(opts ...func(*LicenseTypeQuery)) *DeviceQuery {
	query := (&LicenseTypeClient{config: dq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	dq.withLicenseType = query
	return dq
}

// WithCreator tells the query-builder to eager-load the nodes that are connected to
// the "creator" edge. The optional arguments are used to configure the query builder of the edge.
func (dq *DeviceQuery) WithCreator(opts ...func(*UserQuery)) *DeviceQuery {
	query := (&UserClient{config: dq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (dq *DeviceQuery) WithUpdater(opts ...func(*UserQuery)) *DeviceQuery {
	query := (&UserClient{config: dq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (dq *DeviceQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Device, error) {
	...
	if query := dq.withLicenseType; query != nil {
		if err := dq.loadLicenseType(ctx, query, nodes, nil,
			func(n *Device, e *LicenseType) { n.Edges.LicenseType = e }); err != nil {
			return nil, err
		}
	}
	if query := dq.withCreator; query != nil {
		if err := dq.loadCreator(ctx, query, nodes, nil,
			func(n *Device, e *User) { n.Edges.Creator = e }); err != nil {
			return nil, err
		}
	}
	if query := dq.withUpdater; query != nil {
		if err := dq.loadUpdater(ctx, query, nodes, nil,
			func(n *Device, e *User) { n.Edges.Updater = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (dq *DeviceQuery) loadLicenseType(ctx context.Context, query *LicenseTypeQuery, nodes []*Device, init func(*Device), assign func(*Device, *LicenseType)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Device)
	for i := range nodes {
...
func (dq *DeviceQuery) loadCreator(ctx context.Context, query *UserQuery, nodes []*Device, init func(*Device), assign func(*Device, *User)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Device)
	for i := range nodes {
...
func (dq *DeviceQuery) loadUpdater(ctx context.Context, query *UserQuery, nodes []*Device, init func(*Device), assign func(*Device, *User)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Device)
	for i := range nodes {
...
func (dq *DeviceQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := dq.ctx.Fields; len(fields) > 0 {
		...
		if dq.withProduct != nil {
			_spec.Node.AddColumnOnce(device.FieldProductID)
		}
		if dq.withLicenseType != nil {
			_spec.Node.AddColumnOnce(device.FieldLicenseTypeID)
		}
